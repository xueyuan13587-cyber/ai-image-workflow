...
export async function storeSetNxEx<T>(key: string, value: T, ttlSeconds: number) {
  const serialized = JSON.stringify(value);
...
export async function storeListPopRight(key: string) {
  if (!isRedisConfigured()) {
    const list = memoryLists.get(key) ?? [];
  ...
  return (await redisCommand<string | null>(["RPOP", key])) ?? null;
}
